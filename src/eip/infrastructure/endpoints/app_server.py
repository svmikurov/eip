...
# Проблема
# ~~~~~~~~
# Когда сервер закрывается (или падает), операционная система не
# освобождает порт мгновенно. Он остаётся в состоянии TIME_WAIT на
# некоторое время (обычно от 30 секунд до 4 минут).
...
# Если вы попытаетесь запустить сервер снова сразу после остановки,
# вы получите ошибку: OSError: [Errno 48] Address already in use
#
# Решение
# ~~~~~~~
